...
**目标**：在翻译页签旁新增「语言大师问答」页签，提供基于 PaperHub AI 的多轮对话式语言创作助手。

**实现内容**：

1. **双页签架构**：右侧区域从单一翻译页改为 QTabWidget，包含「翻译」和「语言大师问答」两个页签
2. **ASK 页签 UI**：上中下三区 QSplitter 布局
   - 模板栏（快捷提问按钮 + 管理按钮）
   - 对话区（QTextEdit 只读显示 + Token 圆圈 + 用量标签）
   - 待审核面板（可折叠 + 5列候选词表格 + 全部确认/全部丢弃按钮）
   - 输入区（QPlainTextEdit + 发送按钮 + Ctrl+Enter 快捷键）
3. **快捷提问模板**：6条默认预设（如"帮我生成新词"、"解释语法规则"等）+ 自定义管理对话框 + 持久化到 `app_config.json`
4. **多轮对话引擎**：`_AskChatThread`（QThread）使用 OpenAI SDK 直接多轮调用，流式 chunk 接收实时追加
5. **Token 圆圈可视化**：`_TokenCircleWidget`（环形进度条 + 用量/上限标签），估算当前对话 token 用量
6. **AI 回复解析**：识别【新词】标记格式，提取候选词填入待审核表格
7. **待审核机制**：批量确认（导入词库）/ 批量丢弃 / 清空表格
8. **语言选择器共享**：两个页签共用左侧 language_list，切换语言自动更新系统提示词
9. **窗口尺寸优化**：初始宽度 1480px，Splitter 比例 [200, 1200]

**关键文件**：`app/main_window.py`（新增 `_AskChatThread`, `_TokenCircleWidget`, 14个 `_ask_*` 方法）, `app/paperhub_settings.py`（新增 ASK 模板存取）, `app/ui_theme.py`（新增 ASK 页签样式规则）
...
| 阶段 | 内容 | 优先级 | 对应文档阶段 |
|------|------|--------|------------|
| **阶段十二** | ✅ 语言大师问答页签（多轮对话 + 候选词审核 + 快捷模板） | P0 | AI 对话助手 |
| **阶段十三** | App 主图标定版 + 深色主题设计定版 + 翻译后 AI 朗读按钮 + 系统字体可选项 | P0 | UI 持续改进 |
| **阶段十四** | SSML 增强：`<phoneme>` + `<break>` + voice_settings 映射 + Gender 字段 + 数据格式兼容性修复 | P1-P2 | 阶段三深化 |
| **阶段十五** | ElevenLabs + Resemble AI 语音生成模块 + 音色映射管理 + 冒烟测试流程 | P3 | 阶段五 |
| **阶段十六** | RVC 后处理接口 + 批量流程优化（续翻/缓存/限流） | P4-P5 | 阶段五补充 |
| **阶段十七** | 代码质量：LF 行尾 / Translation_ID / 线程安全 / 标志消费 | P6 | 工程规范 |
